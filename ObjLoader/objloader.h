...
struct material {
	...
	int textureID;	// the id for the texture, if there is no texture than -1
...
class objloader{
	...
	std::vector<unsigned int> textureIDs; // the id for all the textures (so we can delete the textures after use it)
